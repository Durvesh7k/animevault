...
async function uploadToStorage(file, folder = 'posters') {
  ...
  const { data } = supabase.storage.from('anime-posters').getPublicUrl(filename)
...
async function mirrorImageToStorage(remoteUrl) {
  // Use a CORS proxy to fetch the external image as a blob
  const res = await fetch(`https://corsproxy.io/?${encodeURIComponent(remoteUrl)}`)
  if (!res.ok) throw new Error(`Failed to fetch image: ${res.status}`)
  const blob = await res.blob()
  const ext  = blob.type.split('/')[1] || 'jpg'
  const file = new File([blob], `poster.${ext}`, { type: blob.type })
  return uploadToStorage(file, 'api-posters')
}
...
function AddViaAPI({ addAnime, hasAnime }) {
  ...
  const handleAdd = async (anime) => {
    ...
    try {
      // Mirror the API image into Supabase Storage instead of saving external URL
      const remoteUrl = anime.images?.jpg?.large_image_url || anime.images?.jpg?.image_url || ''
      let posterUrl = ''
      if (remoteUrl) {
        toast(`Uploading "${title}" poster…`)
        try {
          posterUrl = await mirrorImageToStorage(remoteUrl)
        } catch {
          // If mirroring fails, fall back to external URL silently
          posterUrl = remoteUrl
        }
      }
